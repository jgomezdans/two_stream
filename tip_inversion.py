...
mu_prior = np.array([0.17, 1, 0.1, 0.7, 2.0, 0.18, 2.0])
mu_prior_snow = np.array([0.17, 1, 0.50, 0.7, 2.0, 0.35, 2.0])

prior_cov = np.diag([0.12, 0.7, 0.0959, 0.15, 1.5, 0.2, 5.0])
prior_cov[2, 5] = 0.8862 * 0.0959 * 0.2
prior_cov[5, 2] = 0.8862 * 0.0959 * 0.2

prior_cov_snow = np.diag([0.12, 0.7, 0.346, 0.15, 1.5, 0.25, 5.0])
...
mu_prior = np.array([0.13, 1, 0.1, 0.77, 2.0, 0.18, 2.0])
mu_prior_snow = np.array([0.13, 1, 0.50, 0.77, 2.0, 0.35, 2.0])

prior_cov = np.diag([0.0140, 0.7, 0.0959, 0.0140, 1.5, 0.2, 5.0])
prior_cov[2, 5] = 0.8862 * 0.0959 * 0.2
prior_cov[5, 2] = 0.8862 * 0.0959 * 0.2

prior_cov_snow = np.diag([0.0140, 0.7, 0.346, 0.0140, 1.5, 0.25, 5.0])
prior_cov_snow[2, 5] = 0.8670 * 0.346 * 0.25
prior_cov_snow[5, 2] = 0.8670 * 0.346 * 0.25
...
def tip_single_inversion(x0, albedo, bu, mu, inv_cov_prior, gp_vis, gp_nir):
    ...
    def cost(x):
        ...
        fwd_vis, dfwd_vis = gp_vis.predict(
            np.atleast_2d(x[vis_posns]), do_unc=False
        )
        fwd_nir, dfwd_nir = gp_nir.predict(
            np.atleast_2d(x[nir_posns]), do_unc=False
        )
        fwd_vis = fwd_vis.squeeze()
        fwd_nir = fwd_nir.squeeze()
        d = fwd_vis - albedo[0]
        der_cost[vis_posns] = der_cost[vis_posns] + d * dfwd_vis / (
            bu[0] ** 2
        )
        cost += 0.5 * d * d / (bu[0] ** 2)

        d = fwd_nir - albedo[1]
        der_cost[nir_posns] = der_cost[nir_posns] + d * dfwd_nir / (
            bu[1] ** 2
        )
        cost += 0.5 * d * d / (bu[1] ** 2)
            ...
            for trial in range(n_tries):
                if trial > 0:
                    while True:
                        ...
                        if np.all(x0 > 0):
                            break
                retval = tip_single_inversion(
                    x0,
                    observations[i, :].squeeze(),
                    bu[i, :].squeeze(),
                    mu,
                    inv_cov_prior,
                    gp_vis,
