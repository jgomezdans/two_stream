...
from eoldas_machinery import retrieve_albedo
...
def tip_single_inversion ( x0, albedo, bu, mu, inv_cov_prior, gp_vis, gp_nir ):
    ...
    def cost (x):
        ...
        der_cost += (x-mu).dot(inv_cov_prior)
        ...
        d = fwd_vis - albedo[0]
        der_cost[vis_posns] = d*dfwd_vis/(bu[0]**2)
        cost += 0.5*d*d/(bu[0]**2)
...
if __name__ == "__main__":
    observations, mask, bu, passer_snow = retrieve_albedo( 2008, "DE-Hai", [0.05, 0.07])
    ...
    for j,tstep in  enumerate(np.arange(1, 366, 8)):
        if tstep in mask[:,0]:
            ...
            cost_list = np.zeros(15)
            solutions = np.zeros((15, 7))
            for trial in xrange(15):
