...
def retrieve_albedo(year, fluxnet_site, albedo_unc, albedo_db="albedo.sql"):
    ...
    passer = np.where(
        np.logical_or(albedo_data.bhr_vis < 0.01, albedo_data.bhr_nir < 0.01),
        False,
        passer,
    )
    ...
    observations = np.c_[
        albedo_data.bhr_vis[passer], albedo_data.bhr_nir[passer]
    ]
    ...
    bu = observations * 0.0
    for i in range(2):
        bu[full_inversions, i] = np.max(
            np.c_[
                np.ones_like(doys[full_inversions]) * 2.5e-3,
                observations[full_inversions, i] * albedo_unc[0],
            ],
            axis=1,
        )
        bu[backup_inversions, i] = np.max(
            np.c_[
                np.ones_like(doys[backup_inversions]) * 2.5e-3,
                observations[backup_inversions, i] * albedo_unc[1],
            ],
            axis=1,
        )
...
class StandardStateTIP(State):
    ...
    def __init__(
        ...
        self.default_values["d_vis"] = 2.0
        self.default_values["a_vis"] = 0.1
        ...
        self.metadata.add_variable(
            "omega_vis",
            "None",
            "Leaf single scattering albedo in visible",
            "omega_vis",
        )
        self.metadata.add_variable(
            "d_vis", "None", "Leaf anisotropy in visible", "d_vis"
        )
        self.metadata.add_variable(
            "a_vis", "None", "Soil albedo in visible", "a_vis"
        )
        self.metadata.add_variable(
            "omega_nir",
            "None",
            "Leaf single scattering albedo in NIR",
            "omega_nir",
        )
        self.metadata.add_variable(
            "d_nir", "None", "Leaf anisotropy in NIR", "d_nir"
        )
        self.metadata.add_variable(
            "a_NIR", "None", "Soil albedo in NIR", "a_nir"
        )
        self.metadata.add_variable("lai", "None", "Effective LAI", "lai_e")
        ...
        max_vals = [0.99, 5.0, 0.99, 0.99, 5.0, 0.99, 6]
        ...
        for (i, param) in enumerate(self.state_config.keys()):
            self.bounds.append(
                [self.parameter_min[param], self.parameter_max[param]]
            )
        ...
        self.cost_history = {"global": [], "iteration": []}
        self.iterations = 0
...
class ObservationOperatorTIP(object):
    ...
    def der_cost(self, x_dict, state_config):
        ...
        for itime, tstep in enumerate(self.state_grid[1:]):
            ...
            sel_obs = np.where(
                np.logical_and(
                    self.mask[:, 0] > istart_doy, self.mask[:, 0] <= tstep
                ),
                True,
                False,
            )
            ...
            sel_obs = np.where(
                np.logical_and(self.mask[:, 1], sel_obs), True, False
            )
            if sel_obs.sum() == 0:
                # We have no observations, go to next period!
                istart_doy = tstep  # Update istart_doy
            ...
            for this_obs_loc in sel_obs.nonzero()[0]:
                ...
                this_cost, this_der, fwd_model, this_gradient = self.calc_mismatch(
                    itime, this_obs, bu
                )
...
def bernards_prior(
    snow_qa, unc_multiplier=1.0, green_leaves=True, use_soil_corr=True, N=46
):
    ...
    prior_mean["a_vis"] = 0.1
    prior_mean["omega_nir"] = 0.77
    prior_mean["d_nir"] = 2.0
    ...
    prior_mean_snow["lai"] = 2.0
    ##########################################################################
    # Prior standard deviation  for NO SNOW case                             #
    ##########################################################################
    prior_inv_cov = OrderedDict()
    prior_inv_cov["omega_vis"] = np.array([0.0140]) * unc_multiplier
    ...
    prior_inv_cov["a_nir"] = np.array([0.2]) * unc_multiplier
    ...
    prior_inv_cov_snow["omega_vis"] = np.array([0.0140]) * unc_multiplier
    ...
    prior_inv_cov_snow["a_nir"] = np.array([0.25]) * unc_multiplier
    ...
    snow_cov = 0.8670 * (
        prior_inv_cov_snow["a_vis"] * prior_inv_cov_snow["a_nir"]
    )
    ...
    for i, parameter in enumerate(prior_mean.keys()):
        xx = np.where(
            snow_qa, prior_mean_snow[parameter], prior_mean[parameter]
        )
        mu_prior[i * N : ((i + 1) * N)] = xx
        xx = np.where(
            snow_qa, prior_inv_cov_snow[parameter], prior_inv_cov[parameter]
        )
...
if __name__ == "__main__":
    ...
    bu = observations * 0.0
    for i in range(2):
        bu[full_inversions, i] = np.min(
            np.c_[
                np.ones_like(doys[full_inversions]) * 2.5e-3,
                observations[full_inversions, i] * 0.05,
            ],
            axis=1,
        )
        bu[backup_inversions, i] = np.min(
            np.c_[
                np.ones_like(doys[backup_inversions]) * 2.5e-3,
                observations[backup_inversions, i] * 0.05,
            ],
            axis=1,
        )
    ...
    for p, v in the_state.default_values.items():
        x_dict[p] = np.random.rand(
            len(state_grid)
        )  # np.ones_like ( state_grid )*v
