...
Results = namedtuple ("Res", "site year doy longitude latitude " + 
...
def process_cli ( ):
    ...
    parser = OptionParser(usage="Jose Gomez-Dans " + \
    "<j.gomez-dans@ucl.ac.uk>\n\n\tUsage: %prog [options]",
    version="%prog 1.0 Jose Gomez-dans <j.gomez-dans@ucl.ac.uk>" )
    
    parser.add_option ("-d", "--db", action="store", dest="db_file", 
                       default="albedo.sql", help="SQLite file to store albedo")
    
    parser.add_option("-l", "--longitude", action="store", default=None,
                    dest="longitude",  help="Longitude (decimal degrees)" )

    parser.add_option("-t", "--latitude", action="store", default=None,
                      dest="latitude", help="Latitude (decimal degrees)" )
...
def download_and_process_obs ( longitude, latitude, year ):
    ...
    year_start = year
    year_end = year
    ( t_step, vis_par1 )= get_modis_data( longitude, latitude, "MCD43A1", \
        "BRDF_Albedo_Parameters_vis.Num_Parameters_01", year_start, year_end )
    ( t_step, vis_par2 )= get_modis_data( longitude, latitude, "MCD43A1", \
        "BRDF_Albedo_Parameters_vis.Num_Parameters_02", year_start, year_end )
    ( t_step, vis_par3 )= get_modis_data( longitude, latitude, "MCD43A1", \
        "BRDF_Albedo_Parameters_vis.Num_Parameters_03", year_start, year_end )
    ws_albedo_vis =  1.0*vis_par1 + 0.189184*vis_par2 -1.377622*vis_par3
    ( t_step, nir_par1 )= get_modis_data( longitude, latitude, "MCD43A1", \
        "BRDF_Albedo_Parameters_nir.Num_Parameters_01", year_start, year_end )
    ( t_step, nir_par2 )= get_modis_data( longitude, latitude, "MCD43A1", \
        "BRDF_Albedo_Parameters_nir.Num_Parameters_02", year_start, year_end )
    ( t_step, nir_par3 )= get_modis_data( longitude, latitude, "MCD43A1", \
    ...
    ( t_step, albedo_qa )= get_modis_data( longitude, latitude, "MCD43A2", \
        "BRDF_Albedo_Quality", year_start, year_end )
        
    ( t_step, snow_qa )= get_modis_data( longitude, latitude, "MCD43A2", \
        "Snow_BRDF_Albedo", year_start, year_end )
        
    return ( t_step, ws_albedo_vis, ws_albedo_nir, albedo_qa, snow_qa, \
        sun_angle )
        ...
        while True:
            ...
            try:
                ...
                for row in [first_row] + result.fetchall():
                    ...
                    snow.append (row[8])
                    sun.append(row[9])
                results = Results ( site=site_name, year=year, 
                            longitude=longitude, latitude=latitude,
                            doy=np.array(t_axis), bhr_vis=np.array(albedo_vis),
                            bhr_nir=np.array(albedo_nir), albedo_qa=np.array(qa),
                            snow_qa=np.array(snow), sun_angle=np.array(sun) )
                return results
                    
            except StopIteration as e:
                # No data for this site and year
                assert longitude is not None, "Longitude must be defined to dload data"
                assert latitude is not None, "Latitude must be defined to dload data"
                longitude = float ( longitude )
                latitude = float ( latitude )
                ( t_step, ws_albedo_vis, ws_albedo_nir, albedo_qa, snow_qa, 
                    sun_angle ) = download_and_process_obs ( 
                                        longitude, latitude, year )         
                data = []
                for i,t in enumerate (t_step):
                    tup = ( site_name, longitude, latitude, year, t - year*1000,
                        float(ws_albedo_vis[i])/1000., float(ws_albedo_nir[i])/1000., 
                        int(albedo_qa[i]), int( snow_qa[i]), float(sun_angle[i] ))
                    data.append (tup)
